...
export class PlanService {
  ...
  async createApplicationPlan(input: CreateApplicationRequest): Promise<ChangePlan> {
  ...
  async backupPlan(application: string): Promise<ChangePlan> {
    const app = await this.resolver.resolve(application);
    return this.createPlan({
      action: 'BACKUP_APPLICATION', risk: 'LOW', summary: `Create a manual backup of ${app.name ?? app.id}`,
  ...
  async restorePlan(application: string, backupId?: string): Promise<ChangePlan> {
    const app = await this.resolver.resolve(application);
    ...
    return this.createPlan({
      action: 'RESTORE_APPLICATION', risk: 'HIGH', summary: `Restore ${app.name ?? app.id} from backup ${selected.id}`,
  ...
  async deletePlan(application: string): Promise<ChangePlan> {
    const app = await this.resolver.resolve(application);
    const backups = await this.dp.listBackups(app);
    return this.createPlan({
      action: 'DELETE_APPLICATION', risk: 'HIGH', summary: `Delete ${app.name ?? app.id}`,
  ...
  private async createPlan(input: { action: PlanAction; risk: RiskLevel; summary: string; target: Record<string, unknown>; proposedInput: Record<string, unknown>; steps: PlanStep[]; preconditions: Preconditions; expectedResult: string; rollback: string; }): Promise<ChangePlan> {
    const now = new Date();
    const base = {
